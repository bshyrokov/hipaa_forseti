...
module "state_bucket" {
  ...
  name       = "bck-hipaa-demo-terraform-state"
...
resource "google_project_iam_binding" "devops_owners" {
  ...
  role    = "roles/owner"
  members = ["group:group_org_admins@cs-gcp-t.com"]
}

# Admin permission at folder level.
resource "google_folder_iam_member" "admin" {
  folder = "folders/909984588844"
  role   = "roles/resourcemanager.folderAdmin"
  member = "group:group_org_admins@cs-gcp-t.com"
}
